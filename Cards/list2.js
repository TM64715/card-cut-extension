...
clearStorageBtn.onclick = function (){
    chrome.storage.local.set({"card2": []}, function () {
      console.log("cleared card2");
    });
    appendTo.innerHTML = "";
  }
  // Export All
  ...
  storagebtn.addEventListener("click", function () {
    ...
    appendTo.innerHTML = "";
    chrome.storage.local.get("card2", function(result) {
      if (typeof result.card2 == "string") {
        cardList2 = result.card2.split('///')
      }
      if (true) {
        console.log("First ones running")  
        cardList2 = result.card2;
        for (let i = (cardList2.length - 1); i >= 0; i--) {
       ...
       ` 
          ...
          $(document).on('click', `#deleteBtn${i}`, function () {
            ...
            chrome.storage.local.set({"card2": cardList2}, function() {
              console.log('Value is set to ' + cardList2);
            })
          })
        }
     }
