...
clearStorageBtn.onclick = function (){
    chrome.storage.local.set({"card1": []}, function () {
      console.log("cleared card1");
    });
    appendTo.innerHTML = "";
  }
  // Export All
  ...
  storagebtn.addEventListener("click", function () {
    ...
    appendTo.innerHTML = "";
    chrome.storage.local.get("card1", function(result) {
      if (typeof result.card1 == "string") {
        cardList1 = result.card1.split('///')
      }
      if (true) {
        console.log("First ones running")  
        cardList1 = result.card1;
        for (let i = (cardList1.length - 1); i >= 0; i--) {
       ...
       ` 
          ...
          $(document).on('click', `#deleteBtn${i}`, function () {
            ...
            chrome.storage.local.set({"card1": cardList1}, function() {
              console.log('Value is set to ' + cardList1);
            })
          })
        }
     }
